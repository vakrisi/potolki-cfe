...
Процедура ЗагрузитьПоставляемыеПравила(ИмяПланаОбмена, ИмяФайлаПравил) Экспорт
	...
	Если ОбменДаннымиСервер.РаспаковатьZipФайл(ИмяФайлаПравил, ИмяВременногоКаталога) Тогда
		...
		Если СписокРаспакованныхФайлов.Количество() = 0 Тогда
			ВызватьИсключение НСтр("ru = 'При распаковке архива не найден файл с правилами.'; en = 'When unzipping archive, file with rules was not found.'");
		КонецЕсли;
...
"RegistrationRules.xml - правила регистрации для текущей программы.'; en = 'Incorrect format of rule set. Expected number of files in the archive is three. Expected files:"
...
"ExchangeRules.xml - правила конвертации для текущей программы;"
"CorrespondentExchangeRules.xml - правила конвертации для программы-корреспондента;"
"RegistrationRules.xml - правила регистрации для текущей программы.'; en = 'File names in the archive are not as expected. Expected files:"
"ExchangeRules.xml - conversion rules for the current application;"
"CorrespondentExchangeRules.xml - conversion rules for the correspondent application;"
"RegistrationRules.xml - registration rules for the current application.'");
	...
	Иначе
		...
		ВызватьИсключение НСтр("ru = 'Не удалось распаковать архив с правилами.'; en = 'Cannot unpack an archive with rules.'");
	КонецЕсли;
	...
	Если ЗагрузкаПравилРегистрации.ФлагОшибки Тогда
		ВызватьИсключение НСтр("ru = 'Ошибка при загрузке правил регистрации.'; en = 'An error occurred when importing registration rules.'");
	КонецЕсли;
...
Процедура УдалитьПоставляемыеПравила(ИмяПланаОбмена) Экспорт
	...
	Для Каждого ВидПравил Из Перечисления.ВидыПравилДляОбменаДанными Цикл
		...
		Если ЕстьОшибки Тогда
			ВызватьИсключение НСтр("ru = 'Ошибка при загрузке правил из конфигурации.'; en = 'An error occurred when importing rules from configuration.'");
		Иначе
...
Процедура ЗагрузитьПравила(Отказ, Запись, АдресВременногоХранилища = "", ИмяФайлаПравил = "", ЭтоАрхив = Ложь) Экспорт
	...
	Если СписокПлановОбмена.Найти(Запись.ИмяПланаОбмена) = Неопределено Тогда
		НСтрока = НСтр("ru = 'План обмена %1 не используется для синхронизации данных, правила не обновлены.'; en = 'Exchange plan %1 is not used for data synchronization, the rules are not updated.'");
	...
	Если ЭтоАрхив Тогда
		...
		Если ОбменДаннымиСервер.РаспаковатьZipФайл(ИмяВременногоАрхива, ИмяВременногоКаталога) Тогда
			...
			// В архиве не было ни одного файла - отказываемся от загрузки.
			Если СписокРаспакованныхФайлов.Количество() = 0 Тогда
				НСтрока = НСтр("ru = 'При распаковке архива не найден файл с правилами.'; en = 'When unzipping archive, file with rules was not found.'");
				ОбменДаннымиСервер.СообщитьОбОшибке(НСтрока, Отказ);
			КонецЕсли;
...
"ExchangeRules.xml - правила конвертации для текущей программы;"
"CorrespondentExchangeRules.xml - правила конвертации для программы-корреспондента.'; en = 'File names in the archive are not as expected. Expected files:"
"ExchangeRules.xml - conversion rules for the current application;"
"CorrespondentExchangeRules.xml - conversion rules for the correspondent application.'");
						ОбменДаннымиСервер.СообщитьОбОшибке(НСтрока, Отказ);
...
"ExchangeRules.xml - правила конвертации для текущей программы;"
"CorrespondentExchangeRules.xml - правила конвертации для программы-корреспондента.'; en = 'Archive contains one file with conversion rules. Expected number of files in the archive is two. Expected files:"
...
"CorrespondentExchangeRules.xml - rules for the correspondent application conversion.'");
				...
				ИначеЕсли СписокРаспакованныхФайлов.Количество() > 1 Тогда
					НСтрока = НСтр("ru = 'При распаковке архива найдено несколько файлов. Должен быть только один файл с правилами.'; en = 'When unzipping archive, multiple files were found. There should be only one file with rules.'");
					ОбменДаннымиСервер.СообщитьОбОшибке(НСтрока, Отказ);
				КонецЕсли;
		...
		Иначе // Если не удалось распаковать файл - отказываемся от загрузки.
			НСтрока = НСтр("ru = 'Не удалось распаковать архив с правилами.'; en = 'Cannot unpack an archive with rules.'");
			ОбменДаннымиСервер.СообщитьОбОшибке(НСтрока, Отказ);
		КонецЕсли;
...
Функция ДвоичныеДанныеИзМакетаКонфигурации(Отказ, ИмяПланаОбмена, ИмяМакета)
	...
	Исключение
		...
		СтрокаСообщения = НСтр("ru = 'Ошибка получения макета конфигурации %1 для плана обмена %2'; en = 'An error occurred when receiving a template of configuration %1 for exchange plan %2'");
...
Процедура ВыполнитьПроверкуЗаполненияПолей(Отказ, Запись)
	...
	Если ПустаяСтрока(Запись.ИмяПланаОбмена) Тогда
		...
		НСтрока = НСтр("ru = 'Укажите план обмена.'; en = 'Specify exchange plan.'");
	...
	ИначеЕсли Запись.ИсточникПравил = Перечисления.ИсточникиПравилДляОбменаДанными.МакетКонфигурации
		...
		НСтрока = НСтр("ru = 'Укажите типовые правила.'; en = 'Specify standard rules.'");
...
Функция ИнформацияОПравилахИзФайла(ИмяФайлаПравил)
	...
	Если Не ((ПравилаОбмена.ЛокальноеИмя = "ПравилаОбмена") И (ПравилаОбмена.ТипУзла = ТипУзлаXML.НачалоЭлемента)) Тогда
		ВызватьИсключение НСтр("ru = 'Ошибка формата правил обмена'; en = 'Error in exchange rule format'");
	КонецЕсли;
