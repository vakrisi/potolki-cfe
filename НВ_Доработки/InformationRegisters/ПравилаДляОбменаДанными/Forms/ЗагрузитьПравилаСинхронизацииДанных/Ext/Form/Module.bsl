...
"или найти в %2'; en = 'Rule set can be downloaded from %1"
"or found in %2'");
	
	ШаблонКаталогОбновлений = НСтр("ru = 'каталоге поставки программы ""%1""'; en = 'directory of the ""%1"" application delivery'");
	ШаблонКаталогОбновлений = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ШаблонКаталогОбновлений, ИмяПрограммы);
	
	ШаблонПользовательскийСайт = НСтр("ru = 'сайта поддержки пользователей системы ""1С:Предприятие 8""'; en = 'support website for users of 1C:Enterprise 8'");
	...
	Если РежимОтладки Тогда
		...
		Если РежимОтладкиВыгрузки Тогда
			...
			Если Не ЗначениеЗаполнено(ИмяФайла) Тогда
				...
				СтрокаСообщения = НСтр("ru = 'Не задано имя файла внешней обработки.'; en = 'Name of external data processor file is not specified.'");
		...
		Если РежимПротоколированияОбменаДанными Тогда
			...
			Если Не ЗначениеЗаполнено(ИмяФайла) Тогда
				...
				СтрокаСообщения = НСтр("ru = 'Не задано имя файла протокола обмена.'; en = 'Exchange protocol file name is not specified.'");
...
Процедура ЗагрузитьПравила(Команда)
	...
	ПараметрыДиалога.Вставить("Заголовок", НСтр("ru = 'Укажите архив с правилами обмена'; en = 'Specify an archive with exchange rules'"));
...
Процедура ВыгрузитьПравила(Команда)
	...
	Если ПустаяСтрока(ЧастиИмени.ИмяБезРасширения) Тогда
		ПолноеИмяФайла = НСтр("ru = 'Правила конвертации'; en = 'Conversion rules'");
	Иначе
	...
	ПараметрыДиалога.Вставить("Заголовок", НСтр("ru = 'Укажите в какой файл выгрузить правила'; en = 'Specify a file the rules will be exported to'") );
...
"Использовать типовые правила конвертации?'; en = 'Rules were not imported from file. If closing, typical conversion rules will be used."
"Use typical conversion rules?'");
			
			Оповещение = Новый ОписаниеОповещения("ЗакрытьФормуЗагрузкиПравил", ЭтотОбъект);
			
			Кнопки = Новый СписокЗначений;
			Кнопки.Добавить("Использовать", НСтр("ru = 'Использовать'; en = 'Use'"));
...
Процедура ЗагрузитьПравилаЗавершение(Знач РезультатПомещенияФайлов, Знач ДополнительныеПараметры) Экспорт
	...
	Если ПустаяСтрока(ТекстОшибки) И ПустаяСтрока(АдресПомещенногоФайла) Тогда
		ТекстОшибки = НСтр("ru = 'Ошибка передачи файла настроек синхронизации данных на сервер'; en = 'An error occurred when transferring the file of data synchronization settings to the server'");
	КонецЕсли;
...
"RegistrationRules.xml - правила регистрации для текущей программы.'; en = 'Incorrect file format of rule set. Zip archive with three files is expected:"
...
Процедура ВыполнитьЗагрузкуПравил(Знач АдресПомещенногоФайла, Знач ИмяФайла, ОписаниеОшибки = Неопределено)
	...
	Если ТипЗнч(ОписаниеОшибки) <> Тип("Булево") И ОписаниеОшибки <> Неопределено Тогда
		...
		Если ОписаниеОшибки.ВидОшибки = "НекорректнаяКонфигурация" Тогда
			Кнопки.Добавить("Отмена", НСтр("ru = 'Закрыть'; en = 'Close'"));
		Иначе
		...
		ПараметрыФормы.ПредлагатьБольшеНеЗадаватьЭтотВопрос = Ложь;
		Если ОписаниеОшибки.ВидОшибки = "НекорректнаяКонфигурация" Тогда
			ПараметрыФормы.Заголовок = НСтр("ru = 'Правила не могут быть загружены'; en = 'Rules cannot be imported'");
		Иначе
			ПараметрыФормы.Заголовок = НСтр("ru = 'Синхронизация данных может работать некорректно'; en = 'Data synchronization may work incorrectly'");
		КонецЕсли;
...
"Перейти в журнал регистрации?'; en = 'Errors occurred when importing the rules."
"Proceed to the event log?'");
	...
	Иначе
		ПоказатьОповещениеПользователя(,, НСтр("ru = 'Правила успешно загружены в информационную базу.'; en = 'Rules were successfully imported to the infobase.'"));
		Закрыть();
	КонецЕсли;
...
Функция ПолучитьАдресВременногоХранилищаАрхиваПравилНаСервере()
	...
	Если Результат.Пустой() Тогда
		...
		НСтрока = НСтр("ru = 'Не удалось получить правила обмена.'; en = 'Cannot receive exchange rules.'");
...
Процедура ОбновитьИнформациюОПравилах()
	...
	Если ИсточникПравил = "ЗагруженныеИзФайла" Тогда
		ИнформацияОбИспользовании = НСтр("ru = 'Используются правила загруженные из файла.'; en = 'Rules imported from file are used.'");
	Иначе
		ИнформацияОбИспользовании = НСтр("ru = 'Используются типовые правила из состава конфигурации.'; en = 'Standard rules from configuration are used.'");
	КонецЕсли;
